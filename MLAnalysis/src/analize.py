...
def plot_partial(predicted_mass_test, actual_mass_test, predicted_mass_train, actual_mass_train, name, steps, show = False):
	...
	plt.hist(predicted_mass_test, bins = 220, range = (30,250), label = 'predicted', alpha = 0.5, color = 'r')
	plt.hist(actual_mass_test, bins = 220, range = (30,250), label = 'actual', alpha = 0.5, color = 'b')
	plt.title('Mass histogram')
	plt.xlabel('Mass')
	plt.ylabel('Count')
	plt.legend()
	plt.savefig('../fig_png/mass/Approach_2/' + name + '_steps_' + str(steps) + '_mass.png')
	if show == True:
		plt.show()
	plt.clf()
	'''
	plt.hist(mass_difference_test, bins = 220, color = 'r')
	plt.title('Mass difference histogram')
	plt.xlabel('Mass difference')
	plt.ylabel('Count')
	plt.savefig('../fig_png/mass_dif/Approach_2/' + name + '_steps_' + str(steps) + '_mass_diff.png')
	if show == True:
		plt.show()
	plt.clf()

	plt.hist(mass_difference_sq_test, bins = 220, color = 'r')
	plt.title('Mass difference histogram')
	plt.xlabel('Squared mass difference')
	plt.ylabel('Count')
	plt.savefig('../fig_png/mass_diff_sq/Approach_2/' + name + '_steps_' + str(steps) + '_mass_diff_sq.png')
	if show == True:
		plt.show()
	plt.clf()
	...
	plt.legend()
	plt.savefig('../fig_png/mass_dif_rel/Approach_2/' + name + '_steps_' + str(steps) + '_mass_diff_relative.png')
	if show == True:
		plt.show()
	plt.clf()
	...
	plt.ylim( (-1,10,) )
	plt.legend()
	plt.savefig('../fig_png/' + name + '_steps_' + str(steps) + '_mass_diff_relative_zoom.png')
	if show == True:
		plt.show()
	plt.clf()
	'''
